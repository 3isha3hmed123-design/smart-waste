...
def resolve_api_key() -> str:
    """إرجاع مفتاح الـ API من session_state أو من متغيرات البيئة."""
...
def configure_gemini(api_key: str):
    """تهيئة عميل Gemini إذا كان المفتاح متوفرًا."""
    ...
    try:
        ...
        return model, True, ""
    except Exception as exc:  # يعتمد على خدمة خارجية
        return None, False, str(exc)


def sync_gemini_state(force_refresh: bool = False):
    """التأكد من أن تهيئة Gemini موجودة في هذه الجلسة."""
...
def render_chatbot():
    ...
    if not st.session_state.get("gemini_configured"):
        ...
        st.info(
            "أدخل مفتاحك في الشريط الجانبي ثم اضغط تحديث الاتصال "
            "أو اضبط متغير البيئة GEMINI_API_KEY."
        )
        return
...
def render_guidelines():
    ...
    with st.expander("تعقيم وتجهيز قبل النقل"):
        st.write(
            "استخدم الأوتوكلاف عند 121° لمدة لا تقل عن 15 دقيقة، وضع ملصقًا يوضح تاريخ التعقيم واسم المشرف."
        )

    with st.expander("النقل والتتبع"):
        st.write(
            "انقل الحاويات في صناديق صلبة مضادة للتسرب مع مستند شحن يوضح مصدر النفايات وجهتها."
        )

    with st.expander("التخزين المؤقت"):
        st.write(
            "احتفِظ بالنفايات المعبأة في مكان بارد ومظلّل بعيدًا عن المرور الكثيف وبمدة تخزين لا تتجاوز 48 ساعة."
        )

    with st.expander("إعادة التدوير/الإتلاف"):
        st.write(
            "بعد التعقيم، يمكن إعادة تدوير المعادن والبلاستيك غير المتحلل ميكروبيًا ضمن مسار منفصل؛ "
            "أما النفايات غير القابلة فَتُتلف بالحرق أو الطمر الآمن."
        )
